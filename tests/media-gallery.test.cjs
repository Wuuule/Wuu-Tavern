...
function fixture(save=true) {
 ...
 const B={id:'B',messages:[
  {id:'m3',role:'user',attachments:[{type:'image',url:'https://example.com/b.png',name:'remote'}]}]};
 ...
 const runtime=new Function('state','window','getActiveConv','isSafeChatImageSource','cloneJsonData','persistState','requestRender',
   ...
   x=>JSON.parse(JSON.stringify(x)),async()=>save,()=>renders++);
 return {runtime,state,A,B,get renders(){return renders;}};
}
...
test('image reuse UI is isolated in the RP helper and supports keyboard close',()=>{
 const rp=fs.readFileSync(path.join(__dirname,'..','ot-rp.js'),'utf8');
 assert.match(rp,/wuu-chat-image-gallery/);
 assert.match(rp,/wuuImageGallery/);
 assert.match(rp,/if \(gallery && !gallery.hidden\) gallery.hidden = true/);
});
