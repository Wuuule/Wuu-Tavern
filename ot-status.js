...
(function () {
    ...
    function playerNames() {
        var n = PLAYER.slice();
        try { if (window.state && state.settings) { if (state.settings.userName) n.push(String(state.settings.userName)); (state.settings.userPersonas || []).forEach(function (p) { if (p && p.userName) n.push(String(p.userName)); }); } } catch (e) {}
        return n;
    }
    ...
    function explode(text) {
        return String(text || '')
            ...
            .replace(/([^
])([沈秋蔡丽白洁周淑雅胡香冷霜韩霜石翠叶星]\S{0,6}\s*\|)/g, '$1\n$2');
    }
    ...
    function parseTables(text) {
        var src = explode(text), out = [], m, re = new RegExp(TABLE_RE.source, 'gi');
        while ((m = re.exec(src))) {
            var name = ((m[1] || '').match(/\bname\s*=\s*["']([^"']+)["']/i) || [])[1] || ('table-' + (out.length + 1));
            var body = String(m[2] || '').trim(); if (body) out.push({ name: name, content: body.slice(0, 12000) });
        }
            ...
            try {
                if (init && typeof init.body === 'string') {
                    ...
                    if (payload && Array.isArray(payload.messages)) {
                        var tables = listTables(), parts = ['【状态中心·只读·禁止打印到正文】'];
                        if (!tables.length) parts.push('当前无表。'); else tables.forEach(function (t) { parts.push('<StatusTable name="' + t.name + '">\n' + t.content + '\n</StatusTable>'); });
                        var msgs = payload.messages.slice();
                        msgs.splice(Math.min(1, msgs.length), 0, { role: 'system', content: parts.join('\n').slice(0, 8000) });
                        payload.messages = msgs; init = Object.assign({}, init, { body: JSON.stringify(payload) });
                    }
                }
            } catch (e) {}
    ...
    function scrubEl(el) {
        ...
        var tables = parseTables(text); if (tables.length) save(tables);
    ...
    function watch() {
        ...
        window.__otStatusObs = obs; scrubAll();
    }
    function esc(s) { return String(s || '').replace(/[&<>"']/g, function (c) { return ({ '&': '&', '<': '<', '>': '>', '"': '"', "'": '&#39;' })[c]; }); }
    function ensureModal() {
        if (document.getElementById('otStatusModal')) return;
        var modal = document.createElement('div'); modal.id = 'otStatusModal';
    ...
    function refreshPanel() {
        ensureModal();
        var listEl = document.getElementById('otStatusTableList'), detail = document.getElementById('otStatusDetail');
        if (!listEl) return;
        ...
        listEl.querySelectorAll('button[data-i]').forEach(function (btn) {
            btn.onclick = function () { var t = tables[Number(btn.getAttribute('data-i'))]; if (detail && t) detail.textContent = t.content || ''; listEl.querySelectorAll('button').forEach(function (b) { b.style.background = '#fff'; }); btn.style.background = '#f0e4d4'; };
        });
        if (detail && tables[0]) detail.textContent = tables[0].content || '';
    }
    function injectFab() {
        if (document.getElementById('otStatusFab')) return;
        var btn = document.createElement('button'); btn.id = 'otStatusFab'; btn.type = 'button'; btn.textContent = '状态中心';
